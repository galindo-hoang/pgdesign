...
/**
 * MinIO File Upload Service Implementation
 * Implements IFileUploadService for MinIO (S3-compatible) storage
 */
export class MinIOFileUploadService implements IFileUploadService {
  ...
  constructor(config?: FileUploadConfig) {
    this.config = {
      ...
      publicEndpoint: process.env.MINIO_PUBLIC_ENDPOINT || `${process.env.MINIO_ENDPOINT || 'localhost'}:${process.env.MINIO_PORT || '9000'}`,
      ...config
    };
    ...
    this.maxFileSize = config?.maxFileSize || parseInt(process.env.MAX_FILE_SIZE || '5242880'); // 5MB default
  }
  ...
  async getFileUrl(objectName: string): Promise<string> {
    try {
      ...
      return `${protocol}://${publicEndpoint}/${bucket}/${objectName}`;
    } catch (error) {
  ...
  extractObjectNameFromUrl(url: string): string {
    try {
      ...
      const bucket = this.config.bucketName || bucketName;
...
export default new MinIOFileUploadService();

// Export class for custom instances
export { MinIOFileUploadService as FileUploadService }; 