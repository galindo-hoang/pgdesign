...
export class ProjectDetailModel extends BaseModel {
  ...
  async validateProjectDetailData(
    data: Partial<CreateProjectDetailRequest>
  ): Promise<string[]> {
    const errors: string[] = [];
    ...
    if (!data.projectCategoryId || data.projectCategoryId <= 0) {
      errors.push(
        "projectCategoryId is required and must be a positive number"
      );
    }
  ...
  private transformDataToRow(
    data: CreateProjectDetailRequest | UpdateProjectDetailRequest
  ): Partial<ProjectDetailRow> {
    ...
    if (data.projectCategoryId !== undefined)
      row.project_category_id = data.projectCategoryId;
    ...
    if (data.contractorName !== undefined)
      row.contractor_name = data.contractorName || null;
    if (data.metaTitle !== undefined) row.meta_title = data.metaTitle || null;
    if (data.metaDescription !== undefined)
      row.meta_description = data.metaDescription || null;
    if (data.tags !== undefined)
      row.tags = data.tags ? JSON.stringify(data.tags) : null;
    if (data.isOnHomePage !== undefined)
      row.is_on_homepage = data.isOnHomePage || false;
  ...
  override async create(
    data: CreateProjectDetailRequest
