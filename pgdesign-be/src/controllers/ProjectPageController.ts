...
export class ProjectPageController {
  ...
  createAboutProjectData = asyncHandler(async (req: Request, res: Response) => {
    ...
    const createdAboutProject =
      await AboutProjectModel.createOrUpdateAboutProject(req.body);
  ...
  updateAboutProjectData = asyncHandler(async (req: Request, res: Response) => {
    ...
    const updatedAboutProject = await AboutProjectModel.update(
      parseInt(id),
      req.body
    );
  ...
  createStatsSectionData = asyncHandler(async (req: Request, res: Response) => {
    ...
    const headerErrors = await StatsSectionModel.validateStatsSectionData(
      statsSection
    );
    if (headerErrors.length > 0) {
      throw createError(
        `Stats section validation errors: ${headerErrors.join(", ")}`,
        400
      );
    }
    ...
    if (statsItems && statsItems.length > 0) {
      for (const item of statsItems) {
        ...
        if (itemErrors.length > 0) {
          throw createError(
            `Stats item validation errors: ${itemErrors.join(", ")}`,
            400
          );
        }
      }
    ...
    const createdStatsSection =
      await StatsSectionModel.createStatsSectionWithItems(
        statsSection,
        statsItems || []
      );
  ...
  updateStatsSectionData = asyncHandler(async (req: Request, res: Response) => {
    ...
    const updatedStatsSection =
      await StatsSectionModel.updateStatsSectionWithItems(
        parseInt(id),
        statsSection,
        statsItems || []
      );
  ...
  getProjectCategoriesData = asyncHandler(
  ...
  createProjectCategoriesData = asyncHandler(
    async (req: Request, res: Response) => {
      const { projectCategories, categories } = req.body;

      const headerErrors =
        await ProjectCategoriesModel.validateProjectCategoriesData(
          projectCategories
        );
      if (headerErrors.length > 0) {
        throw createError(
          `Project categories validation errors: ${headerErrors.join(", ")}`,
          400
        );
      }
      ...
      if (categories && categories.length > 0) {
        for (const category of categories) {
          const categoryErrors =
            await ProjectCategoriesModel.validateProjectCategoryData(category);
          if (categoryErrors.length > 0) {
            throw createError(
              `Project category validation errors: ${categoryErrors.join(
                ", "
              )}`,
              400
            );
      ...
      const createdProjectCategories =
        await ProjectCategoriesModel.createProjectCategoriesWithItems(
          projectCategories,
          categories || []
        );
  ...
  updateProjectCategoriesData = asyncHandler(
    async (req: Request, res: Response) => {
      const { id } = req.params;
      const { projectCategories, categories } = req.body;
      ...
      const updatedProjectCategories =
        await ProjectCategoriesModel.updateProjectCategoriesWithItems(
          parseInt(id),
          projectCategories,
          categories || []
        );
  ...
  deleteProjectCategoriesData = asyncHandler(
    async (req: Request, res: Response) => {
      const { id } = req.params;
  ...
  createProjectCategory = asyncHandler(async (req: Request, res: Response) => {
    const { id } = req.params;
    const categoryData = req.body;

    if (!id) {
      throw createError("Project categories data ID is required", 400);
    }

    const errors = await ProjectCategoriesModel.validateProjectCategoryData(
      categoryData
    );
    if (errors.length > 0) {
      throw createError(
        `Project category validation errors: ${errors.join(", ")}`,
        400
      );
    }

    const createdCategory = await ProjectCategoriesModel.createProjectCategory(
      parseInt(id),
      categoryData
    );
  ...
  updateProjectCategory = asyncHandler(async (req: Request, res: Response) => {
    const { dataId, categoryId } = req.params;
    const categoryData = req.body;

    if (!dataId || !categoryId) {
      throw createError(
        "Project categories data ID and category ID are required",
        400
      );
    }

    const errors =
      await ProjectCategoriesModel.validateUpdateProjectCategoryData(
        categoryData
      );
    if (errors.length > 0) {
      throw createError(
    ...
    const updatedCategory = await ProjectCategoriesModel.updateProjectCategory(
      parseInt(dataId),
      categoryId,
      categoryData
    );
  ...
  deleteProjectCategory = asyncHandler(async (req: Request, res: Response) => {
    const { dataId, categoryId } = req.params;
    ...
    const deleted = await ProjectCategoriesModel.deleteProjectCategory(
      parseInt(dataId),
      categoryId
    );
