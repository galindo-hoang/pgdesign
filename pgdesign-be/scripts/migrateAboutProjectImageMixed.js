...
// Migration script with mixed environment (dev DB + prod VNData)
...
console.log(`   Database: Development (${knexConfig.development.connection.database})`);
console.log(`   VNData Endpoint: ${process.env.VNDATA_S3_ENDPOINT}`);
console.log(`   VNData Bucket: ${BUCKET_NAME}`);
console.log(`   Access Key: ${process.env.VNDATA_ACCESS_KEY ? 'Set' : 'Not set'}`);
console.log('');
    ...
    if (aboutProject) {
      ...
      if (aboutProject.background_image_url) {
        ...
        try {
          ...
          const req = https.get(aboutProject.background_image_url, (res) => {
