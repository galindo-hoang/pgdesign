...
async function updateUrlsToVNDataS3() {
  ...
  try {
    ...
    // Update vision_mission_data
    console.log('📋 Updating vision_mission_data...');
    const visionUpdates = await db('vision_mission_data')
      .where('image', 'like', '%localhost:9000%')
      .update({
        image: db.raw("REPLACE(image, 'http://localhost:9000/pgdesign-assets/', 'https://s3-hcm-r2.s3cloud.vn/pgdesign-new/')"),
        updated_at: new Date()
      });
    console.log(`   ✅ Updated ${visionUpdates} records in vision_mission_data`);
    ...
    totalUpdated = introUpdates + visionUpdates + heroUpdates + diaryUpdates + solutionUpdates + 
                  processUpdates + aboutUpdates + categoryUpdates + projectUpdates + 
                  projectImagesUpdates + serviceHeroUpdates + serviceUpdates;
