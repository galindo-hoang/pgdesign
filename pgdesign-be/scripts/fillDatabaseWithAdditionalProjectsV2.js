...
async function fillDatabaseWithAdditionalProjects() {
  ...
  try {
    // Read and parse the additionalProjectData file
    const filePath = path.join(__dirname, '../../src/services/additionalProjectData.ts');
    const content = fs.readFileSync(filePath, 'utf8');
    
    // Extract the data object using regex
    const dataMatch = content.match(/export const additionalProjectData[^=]*=\s*({[\s\S]*});/);
    if (!dataMatch) {
      throw new Error('Could not parse additionalProjectData');
    }
    ...
    const additionalProjectData = JSON.parse(dataString);
