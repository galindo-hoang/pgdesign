...
async function updateVisionMissionData() {
  ...
  try {
    ...
    // Update vision_mission_data table
    console.log('📋 Updating vision_mission_data...');
    const visionMissionUpdate = await db('vision_mission_data')
      .where('id', 1)
      .update({
        image: mockVisionMissionData.image,
        vision: JSON.stringify(mockVisionMissionData.vision),
        mission: JSON.stringify(mockVisionMissionData.mission),
        core_values: JSON.stringify(mockVisionMissionData.coreValues),
    ...
    for (let i = 0; i < mockVisionMissionData.mission.items.length; i++) {
      await db('mission_items').insert({
        id: i + 1,
        title: mockVisionMissionData.mission.title,
        description: mockVisionMissionData.mission.items[i],
        display_order: i,
        is_active: 1,
        created_at: new Date(),
        updated_at: new Date()
