...
export const fetchServicePageData = async (): Promise<ServicePageData> => {
  ...
  if (USE_MOCK_DATA) {
    console.log(
      "Using mock data for service page (USE_MOCK_DATA flag is enabled)"
    );
...
export const fetchHeroContentData = async (): Promise<HeroContent> => {
  try {
    ...
    return {
      ...
      brandName: "PG DESIGN",
      description:
        "Chúng tôi đồng hành cùng khách hàng từ bản vẽ ý tưởng đến không gian sống hoàn thiện, tối ưu công năng - nâng tầm thẩm mỹ - đảm bảo chất lượng thi công.",
...
export const fetchProcessSection1Data =
  async (): Promise<ServiceProcessData> => {
    try {
      // await delay(200);
      ...
      return {
        ...
        description:
          "PG Design đảm nhận toàn bộ quy trình xây dựng từ phần thô đến hoàn thiện công trình — bao gồm thi công móng, kết cấu, xây tô, ốp lát, sơn nước, lắp đặt thiết bị vệ sinh, hệ thống điện - nước và trần đến hoàn chỉnh.",
...
export const fetchConstructionSection1Data =
  async (): Promise<ConstructionServiceData> => {
    return {
...
export const fetchProcessSection2Data =
  async (): Promise<ServiceProcessData> => {
    try {
      // await delay(200);
      ...
      return {
        ...
        description:
          "PG Design đồng hành cùng bạn từ khâu hoàn thiện công trình, thi công nội thất đến cải tạo lại toàn bộ không gian sống - mang đến sự chỉn chu, tiện nghi và cảm xúc sống trọn vẹn.",
        note: "",
        imageUrl: service2,
      };
    } catch (error) {
...
export const fetchConstructionSection2Data =
  async (): Promise<ConstructionServiceData> => {
    return {
...
export const fetchProcessSection3Data =
  async (): Promise<ServiceProcessData> => {
    try {
      // await delay(200);
      ...
      return {
        ...
        description:
          "Từ khái niệm không gian đến bản vẽ chi tiết, PG Design kiến tạo nên những thiết kế vừa chuẩn công năng, vừa đậm chất thẩm mỹ - thể hiện rõ cá tính và phong cách sống của gia chủ trong từng đường nét.",
        note: "",
        imageUrl: service3,
      };
    } catch (error) {
...
export const fetchConstructionSection3Data =
  async (): Promise<ConstructionServiceData> => {
    return {
...
export const fetchProcessSection4Data =
  async (): Promise<ServiceProcessData> => {
    try {
      // await delay(200);
      ...
      return {
        ...
        description:
          "PG Design nhận thi công các công trình đã có bản vẽ kiến trúc hoặc nội thất, đảm bảo đúng kỹ thuật - đúng thiết kế - đúng tiến độ, mang đến sản phẩm cuối cùng hoàn thiện với chất lượng chuẩn mực.",
        note: "",
        imageUrl: service4,
      };
    } catch (error) {
...
export const fetchConstructionSection4Data =
  async (): Promise<ConstructionServiceData> => {
    return {
