...
export const fetchImageSliderData = async (): Promise<ImageSlideData[]> => {
  ...
  if (
    imageSliderCache.data &&
    now - imageSliderCache.timestamp < CACHE_DURATION
  ) {
...
const fetchImageSliderDataInternal = async (): Promise<ImageSlideData[]> => {
  try {
    ...
    const processedProjects = homePageProjects.map((project) =>
      processProjectImageUrls(project)
    );

    // Convert to ImageSlideData format
    const imageSliderData: ImageSlideData[] = processedProjects.map(
      (project) => ({
        id: project.id,
    ...
    console.log(
      `Converted ${imageSliderData.length} projects to image slider format`
    );
    ...
    if (USE_MOCK_DATA) {
      ...
      return {
        ...
        solutions: data.data.solutions.map((solution: any) => ({
          ...
          title: Array.isArray(solution.title)
            ? solution.title
            : [solution.title],
          link: solution.link || "",
        })),
    ...
    if (USE_MOCK_DATA) {
      ...
      return {
        ...
        workflows: data.data.tabs.map((tab: any) => ({
          id: tab.workflow_key || "",
          icon:
            tab.workflow_key === "design"
              ? DesignProcessIcon
              : ConstructionProcessIcon,
          title: tab.title || "",
          diagram:
            tab.workflow_key === "design"
              ? WorkProcessFlowDiagram1
              : WorkProcessFlowDiagram2,
        })),
      };
    }
  } catch (error) {
    ...
    if (USE_MOCK_DATA) {
      ...
      return {
        ...
        images: [
          ...
          {
            src: projectDiary2,
            alt: "People inspecting a room in construction",
          },
          ...
          {
            src: projectDiary7,
            alt: "Team standing in front of a house design",
          },
...
export const fetchConsultationFormData =
  async (): Promise<ConsultationFormData> => {
    try {
      if (USE_MOCK_DATA) {
        // Use mock data
        // await delay(150);
        ...
        const response = await fetch(
          `${API_BASE_URL}/homepage/consultation-form`,
          {
            method: "GET",
            headers: {
              "Content-Type": "application/json",
        ...
        return {
          title: data.data.main.title || "",
          projectTypes: data.data.projectTypes.map(
            (type: any) => type.name || ""
          ),
    ...
    if (USE_MOCK_DATA) {
      ...
      return {
        ...
        testimonials: {
          ...
          testimonials: apiData.testimonials.testimonials.map(
            (testimonial: any) => ({
              name: testimonial.name || "",
              project: testimonial.project || "",
              text: testimonial.text || "",
        ...
        consultationForm: {
          ...
          projectTypes: apiData.consultationForm.projectTypes.map(
            (type: any) => type.name || ""
          ),
